...
def settingResolver(setting, instance, vpc_data_all, caller_type='AWS', setting_value=False, profile=False, script_config = False):
    if caller_type == 'AWS':
    ...
    for tag in tags:
        ...
        if q_tag == 'flat' and not sg:
            ...
            if tag['Key'].casefold() == q_tag.casefold():
                ...
                if tag['Value'] == 'True'.casefold() or tag['Value'] == "yes".casefold() or tag[
                    'Value'] == "y".casefold():
                    q_tag_value = True
                if tag['Value'] == 'False'.casefold() or tag['Value'] == 'no'.casefold() or tag[
                    'Value'] == "n".casefold():
                    q_tag_value = False
                break
    return q_tag_value
...
def getDOInstances(profile, instance_counter, script_config, cloud_instances_obj_list):
    ...
    for drop in my_droplets:
        ...
        Password = [False, ""]
        Iterm_tags = []
        Docker_context = settingResolver('Docker_context', drop, {}, "DO", False, profile, script_config)
        Instance_use_Ip_public = settingResolver('Use_Ip_public', drop, {}, "DO", True, profile, script_config)
        Instance_use_Bastion = settingResolver('Use_bastion', drop, {}, "DO", False, profile, script_config)
        Or_host_name = settingResolver('Host_name', drop, {}, "DO", False, profile, script_config)
        Bastion = settingResolver('Bastion', drop, {}, "DO", False, profile, script_config)
        Con_username = settingResolver('Con_username', drop, {}, "DO", False, profile, script_config)
        Bastion_Con_username = settingResolver('Bastion_Con_username', drop, {}, "DO", False, profile, script_config)
        Con_port = settingResolver('Con_port', drop, {}, "DO", 22, profile, script_config)
        Bastion_Con_port = settingResolver('Bastion_Con_port', drop, {}, "DO", 22, profile, script_config)
        SSH_key = settingResolver('SSH_key', drop, {}, "DO", False, profile, script_config)
        Use_shared_key = settingResolver('Use_shared_key', drop, {}, "DO", False, profile, script_config)
        Login_command = settingResolver('Login_command', drop, {}, "DO", False, profile, script_config)
        Dynamic_profile_parent_name = settingResolver('Dynamic_profile_parent_name', drop, {}, "DO", False, profile, script_config)
...
def fetchEC2Instance(instance, client, groups, instances, instance_source, reservation, vpc_data_all, profile, script_config):
    instance_vpc_flat_tags = ''
    instance_flat_tags = ''
    Iterm_tags = []
    Password = [False, ""]

    Docker_context = settingResolver('Docker_context', instance, vpc_data_all, 'AWS', False, profile, script_config)
    Instance_use_Bastion = settingResolver('Use_bastion', instance, vpc_data_all, 'AWS', False, profile, script_config)
    Instance_use_Ip_public = settingResolver('Use_Ip_public', instance, vpc_data_all, 'AWS', False, profile, script_config)
    SSH_key = settingResolver('SSH_key', instance, vpc_data_all, 'AWS', instance.get('KeyName', False), profile, script_config)
    Use_shared_key = settingResolver('Use_shared_key', instance, vpc_data_all, 'AWS', False, profile, script_config)
    Login_command = settingResolver('Login_command', instance, vpc_data_all, 'AWS', False, profile, script_config)
    Con_username = settingResolver('Con_username', instance, vpc_data_all, 'AWS', False, profile, script_config)
    Bastion_Con_username = settingResolver('Bastion_Con_username', instance, vpc_data_all, 'AWS', False, profile, script_config)
    Con_port = settingResolver('Con_port', instance, vpc_data_all, 'AWS', 22, profile, script_config)
    Bastion_Con_port = settingResolver('Bastion_Con_port', instance, vpc_data_all, 'AWS', 22, profile, script_config)
    Bastion = settingResolver("Bastion", instance, vpc_data_all, 'AWS', False, profile, script_config)
...
def fetchEC2Region(region, profile_name, instances, groups, instance_source, credentials=False, profile=False, script_config = False):
    ...
    if script_config['AWS'].get('Skip_stopped', True) == False or script_config['Local'].get('Skip_stopped',
                                                                                             True) == False \
            ...
            for instance in reservation['Instances']:
                if script_config["Local"].get('Parallel_exec', True):
                    with concurrent.futures.ProcessPoolExecutor() as executor:
                        future = executor.submit(fetchEC2Instance, instance, client, groups, instances, instance_source,
                                                 reservation, vpc_data_all, profile, script_config)
                        return_value = future.result()
                        print(f'{instance_source}: {return_value}')
                else:
                    print(fetchEC2Instance(instance, client, groups, instances, instance_source, reservation,
                                           vpc_data_all, profile, script_config))
    else:
        print(f'{instance_source}: No instances found in {region}')


def get_MFA_func(profile, role_arn):
    try:
        retry = 3
        while retry > 0:
...
def getEC2Instances(profile, role_arn=False, instance_counter=False, script_config = False, cloud_instances_obj_list = False):
    groups = {}
    instances = {}
    credentials = False
    # global instance_counter
            ...
            while retry > 0:
                try:
                    assumed_role_object = sts_client.assume_role(
                        ...
                        TokenCode=get_MFA_func(profile, profile["role_arns"][role_arn])
                    )
        ...
        for region in profile_dict['instance_by_region']:
            ...
            for instance in profile_dict['instance_by_region'][region]:
                ...
                if instance['SSH_key'] and instance['Use_shared_key']:
                    sharead_key_path = os.path.join(connection_command, os.path.expanduser(
    ...
    for profile_dict in dict_list:
        ...
        for instance in profile_dict['instances']:
            ...
            if "Sorry" in instance:
                connection_command = "echo"
                ip_for_connection = instance
            elif profile_dict["instances"][instance].get('Instance_use_Ip_public', False) == True or not \
            ...
            if profile_dict["instances"][instance]['Bastion'] != False \
                ...
                if profile_dict['instances'][instance]['Bastion_Con_port'] and profile_dict['instances'][instance][
                    'Bastion_Con_port'] != 22:
                ...
                if profile_dict["instances"][instance]['Con_username'] == False and profile_dict["instances"][
                    instance].get('Platform', False) == 'windows':
            ...
            if profile_dict["instances"][instance].get('Platform', '') != 'windows':
                ...
                if profile_dict["instances"][instance]['SSH_key'] and profile_dict["instances"][instance][
                    'Use_shared_key']:
...
def update_statics(CP_OutputDir, script_config):
    profiles = []
    ...
    if os.path.exists(os.path.expanduser(script_config['AWS']['aws_credentials_file'])):
        ...
        for i in config.sections():
            if i not in script_config['AWS']['exclude_accounts']:
                ...
                getEC2Instances(i, instance_counter, script_config)


def DO_Worker(script_config, instance_counter, cloud_instances_obj_list):
    for profile in script_config['DO']['profiles']:
        print(f"Working on {profile['name']}")
        getDOInstances(profile, instance_counter, script_config, cloud_instances_obj_list)
